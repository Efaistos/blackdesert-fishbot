...
public class DeferredStartState extends State {
    ...
    public DeferredStartState(FishBot fishBot) {
        ...
        LOG.info("Bot starting over " + TimeUtils.getStringTime(Application.getConfig().TASK_START().getDelay()));
    }

    @Override
    public void onStep() {

        if (!Application.getConfig().TASK_START().isActive() || timer.isOver(Application.getConfig().TASK_START().getDelay())) {
